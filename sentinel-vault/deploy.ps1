...
stellar keys generate sentinel-owner --network testnet --fund 2>$null
if ($LASTEXITCODE -ne 0) { echo "[INFO] Key 'sentinel-owner' already exists or generation failed (ignoring error)" }

echo "[DEPLOY] Generating AI agent keypair..."
stellar keys generate sentinel-agent --network testnet --fund 2>$null
if ($LASTEXITCODE -ne 0) { echo "[INFO] Key 'sentinel-agent' already exists or generation failed (ignoring error)" }
...
$CONTRACT_ID = stellar contract deploy --wasm target/wasm32-unknown-unknown/release/sentinel_vault.optimized.wasm --source sentinel-owner --network testnet

echo "[DEPLOY] Contract ID: $CONTRACT_ID"

echo "[DEPLOY] Initializing vault..."
stellar contract invoke --id $CONTRACT_ID --source sentinel-owner --network testnet -- initialize --owner $OWNER_ADDR --agent $AGENT_ADDR

echo "[DEPLOY] Setting default risk parameters (Balanced profile)..."
stellar contract invoke --id $CONTRACT_ID --source sentinel-owner --network testnet -- set_risk_params --max_slippage_bps 100 --max_trade_size 5000000000000 --max_daily_trades 10 --confidence_threshold 65

$AGENT_SECRET = stellar keys show sentinel-agent
