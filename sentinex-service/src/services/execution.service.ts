...
async function submitSwapTransaction(
  ...
  try {
    ...
    // We wrap the inner transaction in a Fee Bump transaction sponsored by the Sentinex Service
    if (config.sponsorSecret) {
      log("[SYSTEM]", "--- Gasless Execution: Sponsoring Transaction Fees ---");
      ...
      finalTx = StellarSdk.TransactionBuilder.buildFeeBumpTransaction(
        sponsorKeypair,
        "20000", // Sponsored fee (higher priority)
        transaction,
        StellarSdk.Networks.TESTNET
      );
      
      // Sign the fee bump wrapper
      finalTx.sign(sponsorKeypair);
      log("[TRADE]", `Fee Bump wrapper added. Sponsor: ${sponsorKeypair.publicKey()}`);
    }

    // Submit
    log("[TRADE]", `Submitting ${config.sponsorSecret ? "Gasless" : "Standard"} transaction to Stellar Testnet...`);
    const sendResponse = await server.submitTransaction(finalTx);
